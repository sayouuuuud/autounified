-- =============================================================================
-- Full audit: every button/CTA key + the self-triggered keep-alive mechanism.
-- Read-only where possible; writes are local to test blocks.
-- =============================================================================
\echo ''
\echo '=== PART A: button & link keys used on the live site ==='
...
\echo ''
\echo '=== PART B: simulate the self-triggered keep-alive (lib/keep-alive.ts) ==='
...
DECLARE
  ...
  new_ts   TIMESTAMPTZ;
BEGIN
  -- Step 1: read the current heartbeat exactly like runHeartbeat() does.
  ...
  -- Step 2: simulate the upsert the Node code performs when > 20h have passed.
  ...
  -- Step 3: prove the write actually landed.
  IF NOT EXISTS (
...
\echo ''
\echo '=== PART C: confirm no external cron is referenced anywhere ==='

DO $$
DECLARE
  cron_rows INT;
BEGIN
  -- If pg_cron were installed, this catalog would exist. We want to prove
  -- the database does NOT rely on any scheduled job.
  ...
  IF cron_rows = 0 THEN
    RAISE NOTICE '  OK — no pg_cron jobs registered, keep-alive is app-driven only';
  ELSE
    RAISE NOTICE '  pg_cron present but unused by our app';
  END IF;
END $$;

\echo ''
\echo '=== PART D: final state snapshot ==='
